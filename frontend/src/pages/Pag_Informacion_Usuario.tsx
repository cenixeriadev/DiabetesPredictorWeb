...
export default function Pag_Informacion_Usuario() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        "No se pudo actualizar la información. Por favor, intenta de nuevo más tarde."
      );
    } finally {
      setIsLoading(false);
    }
    ...
    >
      {isLoading ? (
        ...
        <div className="profileinfo-error">
          <p>{error}</p>
          <button onClick={fetchUserInfo} className="profileinfo-retry-button">
            Reintentar
          </button>
        </div>
      ) : (
        <>
          <div className="profileinfo-profile-envoltura">
            <div className="profileinfo-layout-flex">
